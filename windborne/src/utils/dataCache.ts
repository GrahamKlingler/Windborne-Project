...
//   lastModified?: string;
// };
import { getStationPoints, type DataRecord, API_BASE } from './api';

type CacheEntry = { data: DataRecord; ts: number };
...
export async function getDataOnce(
  ...
  // 4) network
  const p = (async () => {
    // hit your backend (already normalized)
    const data = await getStationPoints(id);
  ...
  try {
    ...
    if (local && !opts.forceRefresh) {
      const cached = JSON.parse(local) as CacheEntry;
